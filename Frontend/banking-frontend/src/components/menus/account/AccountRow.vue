...
const emit = defineEmits(["freeze", "unfreeze"])

function freeze(id, accName) {
    if (freeze) {
...
<template>
    <tr>
        ...
        <td class="py-2 px-6 border-b border-gray-200 text-slate-900">
            <button @click="freeze(accountId, name)" class="w-24 shadow-lg hover:bg-blue-600 hover:border-blue-600 focus:ring-2 focus:ring-blue-400 focus:outline-none bg-blue-500 border-blue-500 text-white font-semibold px-2 h-8 border-2 rounded-lg">{{ frozen ? "Unfreeze" : "Freeze" }}</button>
        </td>
    </tr>
</template>